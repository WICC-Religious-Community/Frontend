 ...
 * hero — a considered dark surface (grain + gold glow), never a bare
 * gradient. The header is transparent over this section (see
 * `components/layout/header.tsx`), hence the negative top margin.
 */
    ...
    <section className="on-dark bg-dark relative isolate -mt-[calc(4.5rem+1px)] flex min-h-[max(40rem,100svh)] flex-col justify-end overflow-clip">
      {hero?.media ? (
        ...
        <div aria-hidden="true" className="bg-grain absolute inset-0 -z-20">
          <div className="absolute inset-0 bg-[radial-gradient(90%_70%_at_15%_100%,var(--color-primary-tint-strong),transparent_65%)]" />
          <div className="absolute inset-0 bg-[radial-gradient(60%_50%_at_100%_0%,var(--color-primary-tint),transparent_70%)]" />
        </div>
      )}
      ...
      <Container className="relative pb-16 pt-40 sm:pb-24">
        <div className="animate-in fade-in-0 slide-in-from-bottom-4 max-w-4xl duration-700 motion-reduce:animate-none">
          ...
          {subheadline ? <p className="text-muted mt-6 max-w-2xl text-body-lg text-pretty">{subheadline}</p> : null}
        ...
        {!hasServiceBar ? (
          <span
            aria-hidden="true"
            className="border-on-dark/30 absolute bottom-0 right-(--spacing-gutter) hidden h-14 w-px animate-pulse sm:block"
